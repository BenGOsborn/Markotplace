...
router.post("/owns", protectedMiddleware, async (req, res) => {
    ...
    if (typeof appName === "undefined") return res.status(400).send("App name is required");
...
router.get("/details/:appname", async (req, res) => {
    ...
    if (typeof existingApp === "undefined") return res.status(400).send("Invalid app name");
...
router.get("/dev/details/:appname", protectedMiddleware, devMiddleware, async (req, res) => {
    // Get the user data from the request
    // @ts-ignore
    const { user }: { user: User } = req.locals;
...
// Add an app
router.post("/dev/create", protectedMiddleware, devMiddleware, async (req, res) => {
    // Get the user data from the request
    // @ts-ignore
    const { user }: { user: User } = req.locals;
    ...
    if (typeof exists !== "undefined") return res.status(400).send("An app with that name already exists");
    ...
    if (!detailsSubmitted && price > 0) return res.status(400).send("To charge more than $0 for your app you must first finish setting up your Stripe account");
...
// Edit an app
router.patch("/dev/edit", protectedMiddleware, devMiddleware, async (req, res) => {
    // Get the user data from the request
    // @ts-ignore
    const { user }: { user: User } = req.locals;
    ...
    // Validate the env
    if (typeof env !== "undefined" && !validEnv(env)) return res.status(400).send("Invalid env JSON");
    ...
    if (typeof existingApp === "undefined") return res.status(400).send("No app with this name exists");

    // Check that the dev owns the app
    if (existingApp.dev.id !== user.dev.id) return res.status(401).send("You are not able to edit this app");

    // Set the data to update
    if (typeof title !== "undefined") existingApp.title = title;
    if (typeof description !== "undefined") existingApp.description = description;
    ...
    if (typeof ghRepoOwner !== "undefined" || typeof ghRepoName !== "undefined") {
        try {
            // Delete the existing webhook if the repo was changed
            await axios.delete<any>(`https://api.github.com/repos/${existingApp.ghRepoOwner}/${existingApp.ghRepoName}/hooks/${existingApp.ghWebhookID}`, {
                headers: {
                    Authorization: `token ${user.dev.ghAccessToken}`,
                    Accept: "application/vnd.github.v3+json",
                },
    ...
    if (typeof ghRepoOwner !== "undefined") existingApp.ghRepoOwner = ghRepoOwner;
    if (typeof ghRepoName !== "undefined") existingApp.ghRepoName = ghRepoName;
    if (typeof ghRepoBranch !== "undefined") existingApp.ghRepoBranch = ghRepoBranch;
    if (typeof ghRepoOwner !== "undefined" || typeof ghRepoName !== "undefined" || typeof ghRepoBranch !== "undefined") {
...
router.post("/hook", async (req, res) => {
    ...
    if (typeof hookID === "undefined" || typeof ref === "undefined") return res.status(400).send("Missing webhook ID or ref");
